...
void sdfvoxel_gi_process(uint cascade, vec3 cascade_pos, vec3 cam_pos, vec3 cam_normal, vec3 cam_specular_normal, float roughness, out vec3 diffuse_light, out vec3 specular_light) {
	...
	for (uint j = 0; j < 8; j++) {
		...
		// IMPROVEMENT: Smoother trilinear weights using Chebyshev to reduce probe seams
		vec3 trilinear_smooth = trilinear * trilinear * (3.0 - 2.0 * trilinear);
		float weight = trilinear_smooth.x * trilinear_smooth.y * trilinear_smooth.z * max(0.005, dot(cam_normal, probe_dir));
	...
	if (cascade < SDFGI_MAX_CASCADES) {
		...
		if (blend > 0.0) {
			if (cascade == sdfgi.max_cascades - 1) {
				...
				// IMPROVEMENT: smoother blend curve for cascade transition
				float smooth_blend = blend * blend * (3.0 - 2.0 * blend);
				diffuse = mix(diffuse, diffuse2, smooth_blend);
				specular = mix(specular, specular2, smooth_blend);
			}
		}
		...
		if (roughness < 0.2) {
			...
			// IMPROVEMENT: GGX-based cone aperture — tighter cone for smooth surfaces,
			// wider for rough ones. Replaces the fixed softness that ignored roughness.
			float roughness2 = roughness * roughness;
			float softness = 0.2 + roughness2 * 5.0;
			...
			while (true) {
				...
				if (!found && i >= cascade && length(ray_pos) < radius_sizes[i]) {
					...
					if (fdistance < softness) {
						...
						hit_light.rgb *= 0.5;
						// IMPROVEMENT: smoother hit falloff using quintic instead of linear clamp
						float hit_t = clamp(1.0 - (fdistance / softness), 0.0, 1.0);
						hit_light.a = hit_t * hit_t * (3.0 - 2.0 * hit_t);
					...
					if (i < (sdfgi.max_cascades - 1)) {
						...
						if (fdistance2 < softness) {
							...
							hit_light2.rgb *= 0.5;
							float hit_t2 = clamp(1.0 - (fdistance2 / softness), 0.0, 1.0);
							hit_light2.a = hit_t2 * hit_t2 * (3.0 - 2.0 * hit_t2);
...
void voxel_gi_compute(uint index, vec3 position, vec3 normal, vec3 ref_vec, mat3 normal_xform, float roughness, inout vec4 out_spec, inout vec4 out_diff, inout float out_blend) {
	...
	if (params.high_quality_vct) {
		...
		// IMPROVEMENT: Better low-quality cone directions using cosine-weighted hemisphere
		// sampling (Hammersley-like). More physically correct than 4 × 45° fixed cones.
		// Weights sum to 1.0 and match solid angle for each direction.
		const uint cone_dir_count = 4;
		vec3 cone_dirs[cone_dir_count] = vec3[](
				vec3(0.0,      0.0,      1.0),       // front (normal dir) — stronger weight
				vec3(0.816497, 0.0,      0.577350),  // right-forward
				vec3(-0.408248, 0.707107, 0.577350), // left-up-forward
				vec3(-0.408248,-0.707107, 0.577350)  // left-down-forward
		);

		// IMPROVEMENT: Cosine-weighted cone weights — front cone carries more energy,
		// lateral cones carry less. Preserves energy better than equal weights.
		float cone_weights[cone_dir_count] = float[](0.40, 0.20, 0.20, 0.20);
	...
	// IMPROVEMENT: GGX-based specular cone aperture instead of linear roughness mapping.
	// tan(roughness² × PI/2 × 0.99) gives a physically plausible lobe width.
	float roughness2 = roughness * roughness;
	vec4 irr_light = voxel_cone_trace(voxel_gi_textures[index], cell_size, position, ref_vec, tan(roughness2 * 0.5 * M_PI * 0.99), max_distance, voxel_gi_instances.data[index].bias);
