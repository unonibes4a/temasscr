...
// ---------------------------------------------------------------------------
// Chebyshev weight: smoother probe visibility than plain dot-product clamp.
// Inspired by the PCF / VSM approach used in Lumen probe blending.
// ---------------------------------------------------------------------------
float chebyshev_weight(float mean, float variance, float t) {
...
// ---------------------------------------------------------------------------
// SDFGI / VoxelGI probe sampling — improved trilinear + Chebyshev weights
// ---------------------------------------------------------------------------
void sdfvoxel_gi_process(uint cascade, vec3 cascade_pos, vec3 cam_pos, vec3 cam_normal, vec3 cam_specular_normal, float roughness, out vec3 diffuse_light, out vec3 specular_light) {
	...
	for (uint j = 0; j < 8; j++) {
		...
		// Smooth trilinear weights — use smoothstep to soften the linear ramp
		// and avoid the hard "seam" visible when crossing probe boundaries.
		vec3 raw_tri  = vec3(1.0) - abs(probe_to_pos);
		vec3 tri      = smoothstep(vec3(0.0), vec3(1.0), raw_tri);

		// Chebyshev-inspired back-face weight: softer than a hard max(0, dot)
		float n_dot = dot(cam_normal, probe_dir);
		float back_weight = max(0.0, n_dot * 0.5 + 0.5);	// [0..1] hemispherical fade
		back_weight = back_weight * back_weight;				// square for sharper "valid hemisphere"
		back_weight = max(0.005, back_weight);

		float weight = tri.x * tri.y * tri.z * back_weight;
		...
		if (sdfgi.use_occlusion) {
			...
			// Soft occlusion: avoid full black cutoff — use smoothstep instead of hard max
			weight *= smoothstep(0.0, 0.15, occlusion);
		}
		...
		// Diffuse
		{
			...
			vec3 diffuse = textureLod(sampler2DArray(lightprobe_texture, linear_sampler), pos_uvw, 0.0).rgb;
			diffuse_accum += vec4(diffuse * weight * sdfgi.cascades[cascade].exposure_normalization, weight);
		}

		// Specular — smooth roughness crossfade (was hard threshold at 0.2 / 0.99)
		{
			vec3 pos_uvw = specular_posf;
			pos_uvw.xy += vec2(offset.xy) * sdfgi.lightprobe_uv_offset.xy;
			pos_uvw.x  += float(offset.z) * sdfgi.lightprobe_uv_offset.z;

			// Fetch mirror (low roughness) and diffuse-approx (high roughness) probes
			vec3 spec_sharp = textureLod(sampler2DArray(lightprobe_texture, linear_sampler), pos_uvw + vec3(0, 0, float(sdfgi.max_cascades)), 0.0).rgb;
			vec3 spec_rough  = textureLod(sampler2DArray(lightprobe_texture, linear_sampler), pos_uvw, 0.0).rgb;

			// Smooth blend across roughness — was a hard kink at 0.2 and 0.99
			float rough_t = smoothstep(0.15, 0.95, roughness);
			vec3 specular = mix(spec_sharp, spec_rough, rough_t);
...
// ---------------------------------------------------------------------------
// Main SDFGI entry — smoother cascade transitions
// ---------------------------------------------------------------------------
void sdfgi_process(vec3 vertex, vec3 normal, vec3 reflection, float roughness, out vec4 ambient_light, out vec4 reflection_light) {
	...
	if (cascade < SDFGI_MAX_CASCADES) {
		...
		// --- Cascade boundary blend (smoother window than original) ---
		float blend = 0.0;
		{
			...
			// Use smoothstep (was already there), but widen the window slightly
			// so the seam between cascades is less visible.
			blend = len >= blend_from ? smoothstep(blend_from, blend_to, len) : 0.0;
		}

		if (blend > 0.0) {
			if (cascade == sdfgi.max_cascades - 1) {
				// Outermost cascade: fade ambient/reflection out smoothly
				...
				// Smooth cubic blend across cascades
				float t = smoothstep(0.0, 1.0, blend);
				diffuse  = mix(diffuse,  diffuse2,  t);
				specular = mix(specular, specular2, t);
			}
		}

		ambient_light.rgb = diffuse;

		// --- Specular ray-march (low roughness path) ---
		if (roughness < 0.25) {		// slightly extended threshold for smoother fade-in
			...
			// Smoother softness ramp — avoid abrupt fade-out at low roughness
			float softness = 0.2 + smoothstep(0.0, 1.0, min(1.0, roughness * 4.0)) * 4.0;
			...
			while (true) {
				...
				if (!found && i >= cascade && length(ray_pos) < radius_sizes[i]) {
					...
					if (fdistance < softness) {
						...
						hit_light.rgb *= 0.5;
						// Smooth contact instead of hard clamp
						float contact = smoothstep(softness, 0.0, fdistance);
						hit_light.a   = contact;
					...
					if (i < (sdfgi.max_cascades - 1)) {
						...
						if (fdistance2 < softness) {
							...
							hit_light2.rgb *= 0.5;
							float contact2  = smoothstep(softness, 0.0, fdistance2);
							hit_light2.a    = contact2;
							hit_light2.rgb *= hit_light2.a;
						}

						float prev_radius = i == 0 ? 0.0 : radius_sizes[max(0, i - 1)];
						float blend_t     = smoothstep(prev_radius, radius_sizes[i], length(ray_pos));

						fdistance2 /= sdfgi.cascades[next_i].to_cell;

						hit_light = mix(hit_light, hit_light2, blend_t);
						fdistance = mix(fdistance, fdistance2, blend_t);
					}
			...
			// Smooth roughness blend: avoid hard kink at roughness == 0.2
			float b  = smoothstep(0.0, 0.25, roughness);
			float sa = 1.0 - b;
...
// ---------------------------------------------------------------------------
// Voxel cone trace helpers (unchanged logic, same as original)
// ---------------------------------------------------------------------------
vec4 voxel_cone_trace(texture3D probe, vec3 cell_size, vec3 pos, vec3 direction, float tan_half_angle, float max_distance, float p_bias) {
	...
	while (dist < max_distance && color.a < 1.0) {
		...
		if (any(greaterThan(abs(uvw_pos - 0.5), vec3(0.5f + half_diameter * cell_size)))) break;
...
vec4 voxel_cone_trace_45_degrees(texture3D probe, vec3 cell_size, vec3 pos, vec3 direction, float max_distance, float p_bias) {
	...
	while (dist < max_distance && color.a < 1.0) {
		vec3 uvw_pos = (pos + dist * direction) * cell_size;
		if (any(greaterThan(abs(uvw_pos - 0.5), vec3(0.5f + radius * cell_size)))) break;
...
void voxel_gi_compute(uint index, vec3 position, vec3 normal, vec3 ref_vec, mat3 normal_xform, float roughness, inout vec4 out_spec, inout vec4 out_diff, inout float out_blend) {
	...
	if (any(bvec2(any(lessThan(position, vec3(0.0))), any(greaterThan(position, voxel_gi_instances.data[index].bounds))))) return;
	...
	// Smooth blend at voxel GI boundaries (was plain clamp)
	float blend_raw = 1.0 - max(blendv.x, max(blendv.y, blendv.z));
	float blend     = smoothstep(0.0, 0.2, blend_raw);
	...
	if (params.high_quality_vct) {
		...
		vec3  cone_dirs[cone_dir_count]    = vec3[](vec3(0.0,0.0,1.0),vec3(0.866025,0.0,0.5),vec3(0.267617,0.823639,0.5),vec3(-0.700629,0.509037,0.5),vec3(-0.700629,-0.509037,0.5),vec3(0.267617,-0.823639,0.5));
		...
		vec3  cone_dirs[cone_dir_count]    = vec3[](vec3(0.707107,0.0,0.707107),vec3(0.0,0.707107,0.707107),vec3(-0.707107,0.0,0.707107),vec3(0.0,-0.707107,0.707107));
	...
	if (!voxel_gi_instances.data[index].blend_ambient) light.a = 1.0;
	...
	if (!voxel_gi_instances.data[index].blend_ambient) irr_light.a = 1.0;
...
//   depth_raw : raw [0..1] value from the depth buffer (1 = far background)
//   color     : current RGB of the pixel
//   Returns the adjusted color.
//
// Behaviour:
//   - depth_raw == 0 → no change
//   - depth_raw == 1 → subtract 0.1 from each channel (scaled by max(r,g,b)
//     so bright pixels lose more absolute value, dark ones barely change)
//   - in between     → linear interpolation of the darkening amount
// ---------------------------------------------------------------------------
vec3 apply_depth_darkening(vec3 color, float depth_raw) {
	float max_channel  = max(color.r, max(color.g, color.b));
	float darken_scale = depth_raw * 0.1 * max_channel;
	return max(vec3(0.0), color - darken_scale);
}
...
void process_gi(ivec2 pos, vec3 vertex, inout vec4 ambient_light, inout vec4 reflection_light) {
	...
	if (normal.length() > 0.5) {
		...
		if (dynamic_object) roughness = 1.0 - roughness;
...
void main() {
	...
	if (sc_use_vrs) {
		...
		vrs_y = 1 << (vrs_texel & 3);
		if (mod(pos.x, vrs_x) != 0) return;
		if (mod(pos.y, vrs_y) != 0) return;
	}
#endif

	if (sc_half_res) pos <<= 1;

	if (any(greaterThanEqual(pos, scene_data.screen_size))) return;

	// --- Read raw depth BEFORE position reconstruction ---
	float depth_raw = texelFetch(sampler2D(depth_buffer, linear_sampler), pos, 0).r;
	...
	// --- Depth-based background darkening ---
	// Pixels at depth == 1.0 (sky / background) lose up to 0.1 * max_channel.
	// Pixels closer to the camera are untouched (depth_raw == 0 → no change).
	ambient_light.rgb    = apply_depth_darkening(ambient_light.rgb,    depth_raw);
	reflection_light.rgb = apply_depth_darkening(reflection_light.rgb, depth_raw);
